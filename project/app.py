...
with tabs[2]:
    ...
    if not df_daily.empty:
        ...
        daily_series = df_daily["power_demand"].tail(7)
        daily_values = daily_series.values
        start_date = daily_series.index[0] # Cleaner way to get the start date of the slice
...
with tabs[3]:
    ...
    if df_hourly.empty:
        ...
        if not df_daily.empty:
            daily_series = df_daily["power_demand"].tail(7)
            daily_values = daily_series.values
            start_date = daily_series.index[0]
        else:
            # Absolute fallback if daily data is also missing
            daily_values = np.ones(7) * 1000
            start_date = pd.Timestamp.today().normalize() - pd.Timedelta(days=7)
